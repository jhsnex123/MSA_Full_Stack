...
@CrossOrigin(origins = "*")  // *를 주면 누구든 응답반응을 볼 수 있다
public class BookBoardController {
	...
	@PostMapping
	public ResponseEntity<String> register(@RequestBody BookBoard board) { //json형식으로 있는 바디 내용으로 받겠다
		log.info("register 요청");
		boardList.add(board);  // add.(db랑 같은 개념)
