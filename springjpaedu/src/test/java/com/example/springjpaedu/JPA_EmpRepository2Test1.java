...
public class JPA_EmpRepository2Test1 {
   ...
   @Order(3)
    ...
    @Test
    @Order(11)
    void bySalGreaterThan() {   //지정된 값보다 큰(sal > 입력값) 데이터를 조회하는 조건을 뜻합니다. 즉, 특정 필드의 값이 주어진 기준보다 클 때 해당 데이터를 가져오는 쿼리를 자동으로 생성하는 데 사용됩니다.
    	List<Emp> list = empR.findBySalGreaterThan(3000);
    	list.stream().forEach(System.out::println);
    }
    @Test
    @Order(12)
    void bySalGreaterThanEqual() {          //특정 필드의 값이 주어진 값보다 크거나 같은 경우
